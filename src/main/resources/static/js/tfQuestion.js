...
function edit(id) {
    ...
    if (!id) {
        ...
        // 查询数据，回显在编辑框中
        $.post(
            "/community/admin/getTf",
            {"id": id},
            function (data) {
                data = $.parseJSON(data);
                if (data.code === 0) {
                    //把得到的信息存到HTML页面中
                    $("#tfId1").val(data.tf.id);
                    $("#stem1").val(data.tf.stem);
                    //判断图片地址的长度，若为1则该题无图
                    var url = data.tf.imageUrl;
                    if (url.length > 1) {
                        $("#img1").attr("src", url);
                    ...
                    $("#analysis1").val(data.tf.analysis);
                    $("#score1").val(data.tf.score);
                    //开启模态框
                    $("#editTf").modal('show');
                } else {
                    alert(data.msg);
...
function delOne(id) {
    ...
    if (sure) {
        $.post(
            "/community/admin/delTf",
            {"id": id},
            function (data) {
                data = $.parseJSON(data);
                if (data.code === 0) {
                    window.location.href = "/community/admin/tfQuesList";
                } else {
                    alert(data.msg);
                }
            }
